...
if (url.indexOf("afd.baidu.com/afd/entry") != -1 && method == getMethod) {
    ...
    if (body.res == undefined || body.res.splash == undefined) {
        ...
        $notify(notifiTitle, "贴吧-afd", "res-splash字段为undefined");
    } else {
    ...
    if (adCmd == null) {
        console.log("url:" + url);
        $notify(notifiTitle, "贴吧-tiebaads/commonbatch", "adCmd参数为null");
    } else {
    ...
    if (i != -1 && i != url.length - 1) {
        ...
        for (let x = 0; x < arr.length; x++) {
            ...
            if (pair.length == 2) {
                ...
                console.log('url:' + url);
                $notify(notifiTitle, '获取url参数', "pair错误");
            }
        }
    } else {
        console.log('url:' + url);
        $notify(notifiTitle, '获取url参数', "i错误");
        return null;
    }
    // 未匹配到queryName
    return null;
